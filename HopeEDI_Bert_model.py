...
class Hope_with_BERT_class:
    ...
    def data_visualisation(self):
        ...
        # Create a figure with three subplots, all in one row
        plt.figure(figsize=(18, 6))

        # Plot for Training Data
        plt.subplot(1, 3, 1)  # (number of rows, number of columns, index of the subplot)
        sns.countplot(x='labels', data=self.train_data)
        plt.title('Training Set Label Distribution')
        plt.xlabel('Label')
        plt.ylabel('Count')
        # You may adjust the labels to be more descriptive
        plt.xticks(ticks=np.arange(2), labels=['Non-Hope Speech', 'Hope Speech'])

        # Plot for Validation Data
        plt.subplot(1, 3, 2)
        sns.countplot(x='labels', data=self.validation_data)
        ...
        # Plot for Test Data
        plt.subplot(1, 3, 3)
        sns.countplot(x='labels', data=self.test_data)
